...
test('page: served at /r, script from self (CSP hash-free), no inline handlers, a11y basics', () => {
  ...
  assert.match(csp, /connect-src 'self' https:\/\/tseszaprvtvqrkfpditu\.supabase\.co/, 'the page may fetch the function');
...
test('a live token renders the question from the function and posts the tap as JSON, then shows the confirmation', async () => {
  ...
  assert.equal(calls[0].url, `https://tseszaprvtvqrkfpditu.supabase.co/functions/v1/guardian-link?t=${TOKEN}`);
