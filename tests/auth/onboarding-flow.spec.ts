...
test('6. no route returns data without a session (live probe with the publishable key; excuses must name an unapplied migration)', async () => {
  ...
  let reachable = false; try { await fetch('https://aveqjeafghmwafkbbnor.supabase.co/auth/v1/health', { signal: AbortSignal.timeout(5000) }); reachable = true; } catch { /* offline */ }
