    ...
    if (looksLikeClubResearch && !clubResearchDone) {
      try {
        ...
        if (res.error) {
          ...
          reply = `Found ${leads.length} club${leads.length === 1 ? "" : "s"} — the full list with contact info is in the card below.`;
        }
      } catch (e) {
